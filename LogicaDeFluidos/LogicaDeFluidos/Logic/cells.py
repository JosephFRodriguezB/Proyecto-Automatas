...
class Cell:
    ...
    def Set(self,x,y, RenderDownFlowingLiquid, RenderFloatingLiquid):
        self.X = x
        self.Y = y
    ...
    def AddLiquid(self,amount,type="water"):
        self.Liquid += amount
        if type == "water":
            self.Water += amount
        else:
    ...
    def Update(self):
        ...
        # Actualiza el bitmask según el flujo de la cell
        self.Bitmask = 0
        ...
        self.Oil_Scale = min(1,self.Oil)
        self.Water_Scale = min(1,self.Water)
        ...
        if (self.RenderDownFlowingLiquid):
            ...
            if (self.Type == Blank and not self.Top == None and (self.Top.Liquid > MinValue or self.Top.Bitmask == 4)):
                if self.Liquid > MinValue:
                    self.Oil_Scale = self.Oil/self.Liquid
                    self.Water_Scale = self.Water/self.Liquid